...
public class PortfolioController {
	...
	@PostMapping("/post")
	public String portfolioInsert(MultipartFile[] uploadFile, MultipartFile uploadPortfolio, PortfolioVO vo,
			HttpServletRequest request) { // 글 생성 후 포트폴리오 게시판으로 이동
		...
		if (uploadPortfolio != null) {
			...
			File saveFile = new File(
					request.getSession().getServletContext().getRealPath("/resources/files/userportfolio"),
					portfolioFileName);
			try {
	...
	@PostMapping("/update")
	public String portfolioUpdate(MultipartFile[] uploadFile, String[] originalFileName, String[] keepFileName,
			MultipartFile uploadPortfolio, PortfolioVO vo, HttpServletRequest request, Model m) {// 게시글 수정완료 후 본인글로 이동
